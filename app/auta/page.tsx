      ...
      <section style={boxStyle}>
        ...
        {isAdmin && (
          <div style={adminFormStyle}>
            ...
            {editingCar && (
              <button onClick={() => setEditingCar(null)} style={grayButtonStyle}>
                {t.cancel}
              </button>
            )}
          </div>
        ...
        {cars.map((car) => (
          <div
            key={car.id}
            style={isRegistrationWarning(car.registration_until) ? warningCarStyle : carInfoStyle}
          >
            <strong>
              {car.name} {car.plate ? `- ${car.plate}` : ""}
            </strong>
          ...
          {history.length === 0 ? (
            ...
            history.map((h) => (
              <div key={h.id} style={rowStyle}>
                ...
                <span>
                  {h.worker_name} → {h.cars?.name} {h.cars?.plate ? `- ${h.cars.plate}` : ""}
                </span>
              </div>
            ))
          )}
